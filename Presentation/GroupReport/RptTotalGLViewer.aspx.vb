    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        ...
        Try
            ...
            If Not IsPostBack Then
                ...
                str = " select '" + datef + " - " + datet + "' dt,st.SAFT_Code,st.SAFT_Desc,st.SAFT_GLCode,"
                str += " COALESCE(totalAmount,0) transAmount,sg.SAFT_FeeType"
                str += " from SAS_FeeTypes st inner join"
                str += " ( SELECT SAFT_Code,SAFT_GLCode, SAFT_Desc,SAFT_FeeType,SAFT_Hostel,"
                str += " (SUM(CASE tbl.TransType WHEN 'Credit' THEN tbl.transAmount ELSE 0 END)-"
                ...
                str += " from SAS_FeeTypes sf"
                str += " left join SAS_AccountsDetails sd on sd.RefCode = sf.SAFT_Code"
                str += " left join SAS_Accounts sa on  sa.transid = sd.transid"
                str += " group by sf.SAFT_Code, sf.SAFT_GLCode, sf.SAFT_Desc, sf.SAFT_FeeType, sf.SAFT_Hostel, sa.TransType, sa.transdate) AS tbl"
                str += " WHERE SAFT_Code = tbl.SAFT_Code"
                str += datecondition
                str += " group by SAFT_Code, SAFT_GLCode, SAFT_Desc, SAFT_FeeType, SAFT_Hostel) sg "
                str += " on sg.SAFT_Code = st.SAFT_Code"
