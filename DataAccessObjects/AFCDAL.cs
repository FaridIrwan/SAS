...
namespace HTS.SAS.DataAccessObjects
{
    ...
    public class AFCDAL
    {
        ...
        private List<StudentEn> CheckHostelFee(AFCDetailsEn argEn, string sem, string intake)
        {
            ...
            List<StudentEn> loStudEn = new List<StudentEn>();
            SQLSTR = "SELECT * from SAS_Student SS where SS.SASI_PgId='" + argEn.ProgramID + "' and SS.SASI_Faculty = '" + argEn.Faculty +
                     "' and SS.SASI_STATUSRec='1' and SS.SASI_AfcStatus='0' and SS.SASI_CurSemYr = '" + sem + "' and SS.SASI_Intake = '" + intake + 
            ...
            try
            {
                if (!FormHelp.IsBlank(SQLSTR))
                {
                        ...
                        DataBaseConnectionString, SQLSTR).CreateDataReader())
                    {
                        ...
                        {
                            loItem = lostuds.LoadObject(loReader);
                            if (CheckAmountHostel(loItem.MatricNo) == false)
                            {
                                loStudEn.Add(loItem);
                            }
                        }
        ...
        //    List<AccountsDetailsEn> ListAccDetails;
        ...
        //        if (batch == "" || batch == "&nbsp;")
        //        {
        //            // Modified by Syafiq 07 April 2014
        //            string CheckBatchNumber;
        //            CheckBatchNumber = GetAutoNumber("Batch");
        //            if (CheckBatchNumber != "Auto Number")
        //            {
        //                batch = CheckBatchNumber;
        //            }
        //            else
        //            {
        ...
        //        {
        //            loAfcdetails = argEn.AFCDetailslist[i];
        //            List<StudentEn> liststu = new List<StudentEn>();
        //            SQLSTR = "SELECT * from sas_student" +
        //                      " where SASI_PgId='" + loAfcdetails.ProgramID + "'";
        //            if (!string.IsNullOrEmpty(argEn.SAFC_Code))
        //            {
        //                SQLSTR += " and SASI_Faculty = '" + argEn.SAFC_Code + "'";
        //            }
        //            //modified by Hafiz @ 08/6/2016
        //            //commented query that have sast_iscurrentsem = true - start
        //            SQLSTR += " and SASI_PostStatus ='" + argEn.PostStatus + "'" + " AND SASI_StatusRec = '1' and SASI_Intake = '" + argEn.Semester + "' AND SASI_MatricNo in (" + argEn.CreditRef + ")  order by SASI_PgId";
        //            //SQLSTR += " and SASI_PostStatus ='" + argEn.PostStatus + "'" + " AND SASI_StatusRec = '1' and SASI_Intake = '" + argEn.Semester + "' AND SASI_CurSemYr IN (select Sast_Code from sas_semestersetup where sast_iscurrentsem = true) AND SASI_MatricNo in (" + argEn.CreditRef + ")  order by SASI_PgId";
        //            //commented query that have sast_iscurrentsem = true - end
        ...
        //                        else
        //                        {
        //                            loItem.SASI_AFCStatus = true;
        //                        }
        //                        if (loItem.MatricNo == "")
        //                        {
        //                            loItem.SASI_AFCStatus = false;
        //                            throw new Exception("No Student in Current Semester in Setup Structure ");
        //                        }
        //                        else
        //                        {
        //                            student = true;
        //                            sMatricNo = loItem.MatricNo.ToString();
        //                        }

        //                        ListAccDetails = new List<AccountsDetailsEn>();
        //                        Double sumAccDetails = 0;
        //                        double total2 = 0.0;//tuition fee
        //                        //Added Mona 4/3/2016
        //                        //string FeeCode = "";
        //                        //double tuitionAmt = 0.0;
        //                        AccountsDetailsEn loItema = new AccountsDetailsEn();

        //                        string FeeQuery;
        //                        //FeeQuery = " SELECT SAS_FeeStrAmount.SAFS_Code, SAS_FeeStrAmount.SAFA_Amount, SAS_FeeStrAmount.SAFD_Type,SAS_FeeStrAmount.SAFT_Code, SAS_FeeTypes.SAFT_IsTutionFee,SAS_FeeStruct.sast_code FROM SAS_FeeStrAmount" +
        //                        FeeQuery = " SELECT SAS_FeeStrAmount.SAFS_Code, SAS_FeeStrAmount.SAFA_Amount, SAS_FeeStrAmount.SAFD_Type,SAS_FeeStrAmount.SAFT_Code,SAS_FeeStruct.sast_code,SAS_FeeStrAmount.safd_sem FROM SAS_FeeStrAmount" +
        //                                    " INNER JOIN SAS_FeeStrDetails ON SAS_FeeStrAmount.SAFS_Code = SAS_FeeStrDetails.SAFS_Code INNER JOIN SAS_FeeStruct ON" +
        //                                    " SAS_FeeStrDetails.SAFS_Code = SAS_FeeStruct.SAFS_Code INNER JOIN SAS_FeeTypes ON SAS_FeeStrDetails.SAFT_Code = SAS_FeeTypes.SAFT_Code WHERE (SAS_FeeStrAmount.SASC_Code = '" + loItem.FeeCat + "')" +
        //                            //" and SAS_FeeStruct.sapg_code = '" + loItem.ProgramID + "' and SAS_FeeStruct.sast_code = '" + loItem.CurretSemesterYear + "'" +
        //                            //" and SAS_FeeStruct.sabp_code IN (SELECT sabp_code FROM sas_program WHERE sapg_code = '" + loItem.ProgramID + "') and SAS_FeeStruct.sast_code = '" + loItem.CurretSemesterYear + "'" +
        //                                     " and SAS_FeeStruct.sabp_code IN (SELECT sabp_code FROM sas_program WHERE sapg_code = '" + loItem.ProgramID + "') and SAS_FeeStruct.sast_code = '" + loItem.Intake + "'" +
        ...
        //                                    "ORDER BY SAS_FeeStrAmount.SAFD_Type";

        //                        if (!FormHelp.IsBlank(FeeQuery))
        //                        {
        //                            using (IDataReader loReadera = _DatabaseFactory.ExecuteReader(Helper.GetDataBaseType,
        //                                    DataBaseConnectionString, FeeQuery).CreateDataReader())
        //                            {
        //                                //Fee items for the particular student
        //                                while (loReadera.Read())
        //                                {
        //                                    double total = 0.0; //semester fee
        //                                    double total1 = 0.0;//admin fee                                            
        //                                    //AccountsDetailsEn loItema = new AccountsDetailsEn();
        //                                    double feeamount = 0.0;

        //                                    {
        //                                        loItema.ReferenceCode = GetValue<string>(loReadera, "SAFT_Code");
        //                                        loItema.TransStatus = "Open";
        //                                        loItema.PostStatus = "Posted";
        //                                        //loItema.PostStatus = "Ready";    

        //                                        //if (GetValue<int>(loReadera, "SAFT_IsTutionFee") == 1)
        //                                        //{
        //                                        //    loItema.TransactionAmount = GetValue<double>(loReadera, "SAFA_Amount") * loItem.SASI_CrditHrs;
        //                                        //    loItema.TempAmount = GetValue<double>(loReadera, "SAFA_Amount") * loItem.SASI_CrditHrs;
        //                                        //}
        //                                        //else
        //                                        //{
        //                                        //loItema.TransactionAmount = GetValue<double>(loReadera, "SAFA_Amount");
        //                                        //loItema.TempAmount = GetValue<double>(loReadera, "SAFA_Amount");
        //                                        //}                                                
        //                                        string SAFD_TYPE = (GetValue<string>(loReadera, "SAFD_Type"));
        //                                        if (SAFD_TYPE == "A")
        //                                        {
        //                                            if (loItem.CurrentSemester == 1)
        //                                            {
        //                                                feeamount = GetValue<double>(loReadera, "SAFA_Amount");
        //                                                //Adding Total Amount
        //                                                total1 += GetValue<double>(loReadera, "SAFA_Amount");
        //                                            }
        //                                        }

        //                                        if (SAFD_TYPE == "S")
        //                                        {
        //                                            if (GetValue<int>(loReadera, "safd_sem") == 0)
        //                                            {
        //                                                feeamount = GetValue<double>(loReadera, "SAFA_Amount");
        //                                                //Adding Total Amount
        //                                                total += GetValue<double>(loReadera, "SAFA_Amount");
        //                                            }
        //                                            if (GetValue<int>(loReadera, "safd_sem") == loItem.CurrentSemester)
        //                                            {
        //                                                feeamount = GetValue<double>(loReadera, "SAFA_Amount");
        //                                                //Adding Total Amount
        //                                                total += GetValue<double>(loReadera, "SAFA_Amount");
        ...
        //                                            //If else to check student master or degree program then calculate credit hours X credit amount
        //                                        }

        //                                        sumAccDetails += total + total1;

        //                                        if (SAFD_TYPE == "T")
        //                                        {
        //                                            //Get Tuition Fee
        //                                            total2 = Convert.ToDouble(loStd.GetTuitionFee(loItem.Intake, loItem.ProgramID, loItem.SASI_CrditHrs));
        //                                            feeamount = total2;
        //                                        }

        //                                        loItema.TransactionAmount = feeamount;
        //                                        ListAccDetails.Add(loItema);
        //                                    }
        ...
        //                        ////Get Tuition Fee
        //                        //total2 = Convert.ToDouble(loStd.GetTuitionFee(loItem.Intake, loItem.ProgramID, loItem.SASI_CrditHrs));                                
        //                        //loItema.TransactionAmount = total2;
        //                        //ListAccDetails.Add(loItema);

        //                        sumAccDetails += total2;

        //                        loen.CreditRef = loItem.MatricNo;
        //                        loen.MatricNo = loItem.MatricNo;
        //                        //Get Hostel/Koko                                 
        //                        loen.IsHostel = loItem.Hostel;
        //                        loen.IsKoko = loItem.KokoCode;

        //                        loen.TempAmount = sumAccDetails;
        //                        loen.TransactionAmount = sumAccDetails;
        //                        loen.AccountDetailsList = ListAccDetails;
        //                        //Insert Invoice
        //                        loen.BatchCode = batch;
        //                        if (ListAccDetails.Count > 0)
        //                        {
        //                            lods.Insert(loen);
        //                            IsRecordInserted = true;
        //                            if (loen.PostStatus == "Posted")
        //                            {
        //                                //Update OutStanding Amount - BY JK
        ...
        //                        if (RecStatus == "Posted")
        //                        {
        //                            //UpdateStudentAFCStatus(loen.CreditRef, argEn.Semester, argEn.AFCDetailslist[0].ProgramID, 2, sMatricNo);
        //                            UpdateStudentAFCStatus(loen.CreditRef, argEn.CurrentSemester, argEn.AFCDetailslist[0].ProgramID, 2, sMatricNo);
        //                        }
        //                        else
        //                        {
        //                            //UpdateStudentAFCStatus(loen.CreditRef, argEn.Semester, argEn.AFCDetailslist[0].ProgramID, 1, sMatricNo);
        //                            UpdateStudentAFCStatus(loen.CreditRef, argEn.CurrentSemester, argEn.AFCDetailslist[0].ProgramID, 1, sMatricNo);
        //                        }
        //                    }
        //                    loReader.Close();
            ...
            try
            {
                ...
                if (!FormHelp.IsBlank(sqlCmd))
                {
                    ...
                    if (liRowAffected > -1)
                    {
                        ...
                        sqlCmd = "select sas_accounts.creditref,sas_afc.semester,sas_afcdetails.programcode " +
                                        "from sas_accounts inner join sas_afc on sas_accounts.batchcode = sas_afc.batchcode " +
                                        "inner join sas_afcdetails on sas_afc.transcode = sas_afcdetails.transcode " +
                        ...
                        if (!FormHelp.IsBlank(sqlCmd))
                        {
                            ...
                            while (loReader.Read())
                            {
                                if (loReader.RecordsAffected != -1)
                                {
                                    ...
                                    Program = GetValue<string>(loReader, "programcode");
